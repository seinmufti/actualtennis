...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, name: 'Actual Tennis' })
})
...
app.get('/api/bookings', (req, res) => {
  ...
  const bookings = date ? store.getByDate(date) : store.getAll()
  const deviceId = deviceIdFrom(req)
  const mine = deviceId ? store.getByDevice(deviceId) : null
...
app.post('/api/bookings', (req, res) => {
  ...
  const taken = store
    .getByDate(date)
    .some((booking) => booking.courtId === courtId && booking.hour === hour)
  if (taken) {
